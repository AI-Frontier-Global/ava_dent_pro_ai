...
export default function SignupPage({ onBack, onSuccess, selectedPlan, signIn, signInWithGoogle }: Props) {
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-900 p-4" dir="rtl">
      ...
      <div className="relative z-10 w-full max-w-lg">
        <div className="rounded-3xl border border-white/20 bg-white/95 p-8 shadow-elev-4 backdrop-blur-xl sm:p-10">
          ...
          {/* Google Sign-Up — secondary option */}
          <div className="mt-6">
            <div className="flex items-center gap-3">
              <div className="h-px flex-1 bg-white/20" />
              <span className="text-xs font-medium text-sky-100/70">أو</span>
              <div className="h-px flex-1 bg-white/20" />
            </div>
            <p className="mt-3 mb-3 text-center text-xs text-sky-100/60">
              يمكنك إنشاء حساب ببريدك الإلكتروني وكلمة المرور، أو استخدام حساب Google الخاص بك.
            </p>
            <button
              type="button"
              onClick={async () => { await signInWithGoogle(); }}
              className="flex w-full items-center justify-center gap-3 rounded-xl border border-white/30 bg-white/10 px-6 py-3.5 text-sm font-semibold text-white backdrop-blur-sm transition-all hover:bg-white/20 active:scale-[0.98]"
            >
